...
#
# This version is modified to output multiples of axis_step (e.g., -10, 0, 10).
# ============================================================
...
def _safe_write_pickle(obj, dest_path: str):
    ...
    tmp_file = str(tmp_dir / f"_interp_tmp_{os.getpid()}_{random.randint(0,1_00_000)}.pkl")
...
class UnitStepInterpolator:
    def __init__(self, k=133, max_steps=None, axis_step: float = 1.0):
        """
...
def run(input_pkl: str, output_pkl: str, batch_size: int = 50, verify_sample: int = 80,
        kpoints: int = 133, max_steps: int = None, seed: int = 42, axis_step: float = 1.0):
    ...
    # We will get the actual axis_step from the interpolator
    s = interp.axis_step
    print(f"Processing with axis_step={s}. Output will be multiples of {s}.")
    ...
    for i in range(0, N, batch_size):
        ...
        for k in batch_keys:
            ...
            if not (isinstance(entry, dict) and "keypoint" in entry):
                continue
            ...
            # We round the diff to the nearest multiple of s (axis_step).
            # e.g., if s=10: diff=10 -> 10, diff=4.9 -> 0, diff=5.1 -> 10
            # Shape: (T_interp-1, K, 2)
            motion_vectors_dx_dy = (np.round(diff / s) * s).astype(np.int8)
            # --- END MODIFIED LOGIC ---
